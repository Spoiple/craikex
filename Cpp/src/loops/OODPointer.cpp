#include "OODPointer.h"

namespace exj {

    OODPointer::OODPointer(sf::RenderWindow *window, int numberOfEntities, int width, int height) {
        this->window = window;
        this->width = width;
        this->height = height;
        ...
        for (int i = 0; i < numberOfEntities; i++) {
            entities[i] = new exj::Rectangle{static_cast<float>(rand() % width), static_cast<float>(rand() % height), 10, 10, static_cast<float>((rand() % 160) - 80), static_cast<float>((rand() % 160) - 80), false, static_cast<float>(rand() % 255), static_cast<float>(rand() % 255), static_cast<float>(rand() % 255)};
        }
    }

    void OODPointer::update(float dt) {
        for (int i = 0; i < numberOfEntities; i++) {
            entities[i]->move(dt);
        }
        for (int i = 0; i < numberOfEntities; i++) {
